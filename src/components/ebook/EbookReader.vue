...
import {
  ...
  saveTheme, getTheme,
  getLocation, setBookObject
} from '../../utils/localStorage'
import Epub from 'epubjs'
import { getDevLocation } from '@/api/store'
global.epub = Epub
export default {
  mixins: [ebookMixin],
  mounted () {
    ...
    getLocalForage(fileName, (err, blob) => {
      if (!err && blob) {
        ...
        console.log('在线获取电子书')
        this.setFileName(books.join('/')).then(() => {
          console.log('图书url' + this.fileName)
  ...
  methods: {
    ...
    initRendition () {
      ...
      let devgetLocaltion = null
      getDevLocation(this.fileName).then(response => {
        if (response.status === 200 && response.data.result) {
          devgetLocaltion = response.data.result.location
          if (!(devgetLocaltion || localStorage.getLocalStorage(`${this.fileName}-NE`) === 'ture')) {
            location = devgetLocaltion
            setBookObject(this.fileName, response.data.result.readTime)
          }
        }
      })
    ...
    initEpub (url) {
      ...
      this.book.ready.then(() => {
        ...
        locations.forEach(item => {
          ...
          this.navigation.forEach(nav => {
            if (nav.href) {
              console.log('navhref' + nav.href)
              const href = nav.href.match(/^(.*)\.xhtml$/)[1]
              console.log('href:' + href)
              if (href) {
                console.log('href1:' + href[1])
                if (href[1] === loc) {
