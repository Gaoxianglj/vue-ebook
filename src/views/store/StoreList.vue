...
// import { keywordLog, list } from '../../api/store'
...
export default {
  ...
  methods: {
    ...
    getList () {
      console.log('cccategory2233')
      list().then(response => {
        this.list = response.data.result
        this.total = response.data.total
        const category = this.$route.query.category
        const keyword = this.$route.query.keyword
        console.log('cccategory222', category)
        if (category) {
          console.log('cccategory', category)
          const key = Object.keys(this.list).filter(key1 => {
            console.log('book:', key1)
            this.list[key1] = this.list[key1].filter(book =>
              book.categoryText === category)
            return this.list[key1].length > 0
          })[0]
          console.log('cccategory11111', key)
