...
export default {
  ...
  methods: {
    ...
    async recommendBook () {
      ...
      console.log('data:' + data.result.random[0].title)
      if (data) {
        const randomIndex = Math.floor(Math.random() * data.result.random.length)
        // 生成随机推荐的一本书
        this.random = data.result.random[randomIndex]
        this.banner = data.result.banner
        this.guessYouLike = data.result.guessYouLike
        this.recommend = data.result.recommend
        this.featured = data.result.featured
        this.categoryList = data.result.categoryList
        this.categories = data.result.categories
        console.log(data.result.random[1].title)
      }
    }
  }
