...
import { addShelf, detail, removeShelf } from '../../api/store'
...
export default {
  ...
  methods: {
    ...
    addOrRemoveShelf () {
      if (this.inBookShelf) {
        this.setShelfList(removeFromBookShelf(this.bookItem)).then(() => {
          ...
          removeShelf(this.bookItem)
        })
      } else {
        addToShelf(this.bookItem)
        this.setShelfList(getBookShelf())
        addShelf(this.bookItem)
      }
    },
    showToast (text) {
      this.toastText = text
    ...
    readBook () {
      console.log('readBook:' + this.privateBook)
      this.$router.push({
        path: `/ebook${this.privateBook}|${this.fileName}`
      })
    },
    trialListening () {
      ...
      if (this.fileName) {
        detail({
          ...
          if (response.status === 200 && response.data.result) {
            ...
            this.bookItem.fileName = this.fileName
            this.privateBook = this.bookItem.privateBook
            let rootFile = data.rootFile
            if (rootFile.startsWith('/')) {
              rootFile = rootFile.substring(1, rootFile.length)
            }
            this.opf = `${process.env.VUE_APP_EPUB_OPF_URL}${this.privateBook}/${this.fileName}${rootFile}`
